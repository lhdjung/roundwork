...
# definition of each procedure, not from any roundwork function.
...
test_that("all rounding functions share the same tolerance", {
  ...
  # implicit until roundwork 0.1.0, when the `*_from()` functions expressed it
  # as `threshold - .Machine$double.eps^0.5` instead -- which the `/ 10` in the
...
test_that("`round_ties_*()` break ties as their names say", {
  # Hand-computed, not taken from any roundwork function:
...
test_that("the IEEE 754 spelling is the same function", {
  # `round_ties_to_away()` and `round_ties_away()` are two names for
  # *roundTiesToAway*, and the other IEEE names likewise duplicate procedures
  # that exist under non-standard names. Keeping both only works if they cannot
  # drift apart:
  x <- c(mid, -mid, seq(-500, 500) / 100)
  expect_equal(round_ties_to_away(x, 1), round_ties_away(x, 1))
  expect_equal(round_ties_to_even(x, 1), round(x, 1))
  expect_equal(round_toward_positive(x, 1), round_ceiling(x, 1))
  expect_equal(round_toward_negative(x, 1), round_floor(x, 1))
  expect_equal(round_toward_zero(x, 1), round_trunc(x, 1))
})
