...
Rounding is a more diverse and variegated topic than one might think. "Rounding" is often taken to simply mean rounding up from 5, and although that's wrong, it doesn't make much of a difference most of the time.
...
This vignette goes into depth on roundwork's infrastructure for reconstructing rounding procedures and results from summary statistics. All of that is essential for packages built on top of roundwork, such as [scrutiny](https://lhdjung.github.io/scrutiny/): it is a precondition for translating assumptions about the processes behind published summary data into a few simple, higher-level function calls.

Feel free to skip the more theoretical parts if your focus is on the code. See `vignette("rounding-options")` for a list of possible values of the `rounding` argument.

## Overview

Base R's `round()` function is surprisingly sophisticated. This distinguishes it from simpler rounding procedures, such as rounding up from 5. For this very reason, however, it can't be used to reconstruct the rounding procedures of other software programs. This is the job of roundwork's rounding functions.
...
`round_up()` and `round_down()` say which way a tie goes *on the number line*, and `symmetric` then decides what that means below zero. Since that translation is easy to get backwards, the same four procedures are also available under names that state them outright:
...
`round_ties_up()` is `round_up()` with `symmetric = FALSE`, and `round_ties_away()` is `round_up()` with `symmetric = TRUE`. Nothing is deprecated by them; use whichever spelling reads better. The `rounding` strings `"ties_up"`, `"ties_down"`, `"ties_away"`, and `"ties_zero"` name the same four procedures inside `reround()` and `unround()`.
...
For reconstruction, two claims about `round()` are easy to run together. `base::round()` is the right way to reconstruct software that rounds binary doubles to even, such as R itself, Python, and NumPy. This is what `rounding = "even"` does. What is unsuitable is the reverse: Predicting `round()`'s output from the decimal you see written down, since most decimals are not exactly representable and so are not really ties at all. Reconstructing the many programs that do round from the decimal 5 is the job of roundwork's other `round_*()` functions. See `vignette("rounding-options")`, which lists which program falls on which side.

### IEEE 754 rounding standard

If you need to follow the IEEE 754 rounding standard, see `round_ties_to_even()` for wrapper functions that implement all five IEEE rounding procedures (or "rounding-direction attributes"). IEEE 754 defines standards for floating-point operations, which includes rounding (IEEE 2019, pp. 27f.; the [Wikipedia page](https://en.m.wikipedia.org/wiki/IEEE_754#Rounding_rules) is more accessible).

Each of them is a wrapper around a function that roundwork already had under a name of its own:

| IEEE 754 function        | Equivalent                            |
|--------------------------|---------------------------------------|
| `round_ties_to_even()`   | `base::round()`                       |
| `round_ties_to_away()`   | `round_ties_away()`, i.e. `round_up()` with `symmetric = TRUE` |
| `round_toward_positive()`| `round_ceiling()`                     |
| `round_toward_negative()`| `round_floor()`                       |
| `round_toward_zero()`    | `round_trunc()`                       |

The duplication is deliberate. roundwork's own names were chosen without the standard in mind, and renaming them would break every downstream caller; but a function that says *roundTowardPositive* on the tin is easier to reach for when a paper or a manual states its rounding in the standard's vocabulary. Note that the standard has no name for the other three tie-breaking procedures, for rounding away from zero (`round_anti_trunc()`), or for the custom thresholds of `round_up_from()`.
...
Alternatively, a function that uses `unround()` as a helper might count decimal places by itself (i.e., by internally calling `scrutiny::decimal_places()`). It should then pass these numbers to `unround()` via the `digits` argument instead of letting it redundantly count decimal places a second time.
...
Deriving a set of candidate values from the decimal bounds above is unsafe. If a product such as `floor(upper * n)` is mathematically an exact integer, its `double` representation can fall on either side of it, so a legitimate candidate may be silently dropped or a phantom one admitted --- and a consistency verdict can flip either way.

`bound_numerators()` therefore returns the same bounds as exact integer numerators over a common denominator, which turns every such comparison into one between integers:
...
This is the low-level entry point that consistency tests build on. Most users want `unround()` instead.
...
```{r echo=FALSE}
up_1 <- rounding_bias(vec6, digits = 1, rounding = "up")
up_2 <- rounding_bias(vec6, digits = 2, rounding = "up")
up_3 <- rounding_bias(vec6, digits = 3, rounding = "up")
up_4 <- rounding_bias(vec6, digits = 4, rounding = "up")
up_5 <- rounding_bias(vec6, digits = 5, rounding = "up")

even_1 <- rounding_bias(vec6, digits = 1, rounding = "even")
even_2 <- rounding_bias(vec6, digits = 2, rounding = "even")
even_3 <- rounding_bias(vec6, digits = 3, rounding = "even")
even_4 <- rounding_bias(vec6, digits = 4, rounding = "even")
even_5 <- rounding_bias(vec6, digits = 5, rounding = "even")
...
up_1 <- rounding_bias(vec7, digits = 1, rounding = "up")
up_2 <- rounding_bias(vec7, digits = 2, rounding = "up")
up_3 <- rounding_bias(vec7, digits = 3, rounding = "up")
up_4 <- rounding_bias(vec7, digits = 4, rounding = "up")
up_5 <- rounding_bias(vec7, digits = 5, rounding = "up")

even_1 <- rounding_bias(vec7, digits = 1, rounding = "even")
even_2 <- rounding_bias(vec7, digits = 2, rounding = "even")
even_3 <- rounding_bias(vec7, digits = 3, rounding = "even")
even_4 <- rounding_bias(vec7, digits = 4, rounding = "even")
even_5 <- rounding_bias(vec7, digits = 5, rounding = "even")
