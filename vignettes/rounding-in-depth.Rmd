...
Rounding is a more diverse and variegated topic than one might think. It is often taken to simply mean rounding up from 5, and although that's wrong, it doesn't make much of a difference most of the time.
...
This vignette goes into depth on roundwork's infrastructure for reconstructing rounding procedures and results from summary statistics. All of that is essential for the broader errorverse: it is a precondition for translating assumptions about the processes behind published summary data into a few simple, higher-level function calls. Some of the functions presented here might be useful for other purposes, as well.

Feel free to skip the more theoretical parts if your focus is on the code. See `vignette("rounding-options")` for a list of possible values of the `rounding` argument in errorverse functions.

## Overview

Base R's `round()` function is surprisingly sophisticated. This distinguishes it from simpler rounding procedures, such as rounding up from 5. For this very reason, however, it can't be used to reconstruct the rounding procedures of other software programs. This is the job of roundwork.
...
`base::round()` is a black box, but it seems unbiased in the long run. I recommend using it for original work, even though it is quite different from other rounding procedures --- and therefore unsuitable for reconstructing them. Instead, we need functions like those in roundwork.

### IEEE 754 rounding standard

If you need to follow the IEEE 754 rounding standard, see `round_ties_to_even()` for handy wrapper functions that implement all five IEEE rounding procedures (or "rounding-direction attributes"). IEEE 754 defines standards for floating-point operations, which includes rounding (IEEE 2019, pp. 27f.; the [Wikipedia page](https://en.m.wikipedia.org/wiki/IEEE_754#Rounding_rules) is more accessible).

The IEEE 754 functions are implemented as wrappers around other rounding functions. This is because roundwork was originally written without this standard in mind. However, it might be useful to have functions that explicitly refer to the standard.
...
Alternatively, a function that uses `unround()` as a helper might count decimal places by itself (i.e., by internally calling `scrutiny::decimal_places()`). It should then pass these numbers to `unround()` via the `decimals` argument instead of letting it redundantly count decimal places a second time.
