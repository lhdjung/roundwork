...
#'   `janitor::round_half_up()` all implement. It is also available under its
#'   official standard name, [`round_ties_to_away()`]; the two are the same
#'   function. `round_ties_up()` is what Stata's `round()` does.
#'   [`base::round()`] is *roundTiesToEven*, reached through `rounding = "even"`
#'   in [`reround()`] and as [`round_ties_to_even()`].
#'   `vignette("rounding-options")` maps each program to a setting.
#'
...
#'   floating-point tolerance. [`round_ties_to_even()`] and friends for the
#'   subset of these procedures that IEEE 754 names. [`round_ceiling()`] for
#'   procedures that ignore which number is nearer.
#'
#' @examples
